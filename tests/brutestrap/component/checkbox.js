import {R,X,inputClassName, switchClassName} from '../externals.js';
...
function checkbox({
  ...
  input = X`
    <section class="checkbox-group ${classNames?classNames.join(' '):''}">
      ${options.map(opt => {
        ...
        return X`
          <label class=row>${optName}<input name=${name} type=checkbox value="${value}"></label>
        `;
      })}
    </section>
  `;
