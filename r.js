...
const XSS = 'Possible XSS attack warning. Possible object forgery attempt detected. Codes do not match.',
  ...
  // Notice that it isn't used
  currentKey = `${Math.random()}`;
Object.assign(self,{R,render});

function R (parts /* [...parts] instead of "parts = Array.from(parts);"?? */, ...vals) {
  parts = Array.from(parts);
  const handlers = {};
  vals = vals.map(v => {
    if (Array.isArray(v) && v.every(item => Boolean(item.handlers) && Boolean(item.str))) {
      return join(v) || '';
    } else if (typeof v === 'object' && Boolean(v)) {
      if (v.str && v.handlers) {
  ...
  while (parts.length > 1) {
    ...
    } else if (Boolean(val) && val.handlers && val.str) {
...
function render (r, root, {'replace': replace = false} = {}) {
  if (Array.isArray(r) && r.every(val => Boolean(val.str) && Boolean(val.handlers))) r = join(r);
  verify(r,currentKey);
  ...
  Object.keys(handlers).forEach(hid => {
    ...
    if (Boolean(node) && Boolean(nodeHandlers)) {
